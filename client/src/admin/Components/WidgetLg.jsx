...
const dispatch = useDispatch()
  ...
  if (orders.length < 5) {
    ...
    slicedOrders= orders.slice(orders.length - 4, orders.length).sort(function(a,b) {
      if (a.id < b.id) {
        return 1;
      }
      if (a.id > b.id) {
    ...
    <div className="widgetLg">
      ...
      <tr className="widgetLgTr">
       ...
       {slicedOrders.map(order => {
         ...
         let user = registeredUser.find(user => user.email === order.userEmail)
         return (    
        <tr className="widgetLgTr">
          <td className="widgetLgUser">
            ...
            />
            <span className="widgetLgName">{user && user.name + ' ' + user.lastName}</span>
          </td>
          <td className="widgetLgDate">{date}</td>
          <td className="widgetLgAmount">{'$' + order.price}</td>
          <td className="widgetLgStatus">
            <Button type={order.status === 'created' ? "Approved" : "Pending"} />
          </td>
        </tr>
         )
