...
import { MailOutline, PermIdentity, Message } from "@material-ui/icons";
        ...
        <div className="userShow">
          <div className="userShowBottom">
            ...
            {reclamos &&
              reclamos?.map((e, i) => {
                return (
                  <div key={i + "d"} className="userShowSuggestion">
                    <div>
                      <PermIdentity className="userShowIcon" />
                      <span key={i + "h"} className="userShowInfoTitle">
                        {e.author}
                      </span>
                    </div>
                    <div>
                      <MailOutline className="userShowIcon" />
                      <span key={i + "p1"} className="userShowInfoTitle">
                        {e.authorEmail}
                      </span>
                    </div>
                    <div>
                    ...
                    <button
                      key={i + "b"}
                      className="userButtonSuggestion"
                      onClick={() => deleteSuggestion(e.id)}
                    >
                      Delete
                    </button>
                  </div>
